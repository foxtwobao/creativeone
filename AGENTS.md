...
- 前端服务请求统一放在 `web/src/services/api/`。本地模式由浏览器直连；显式启用云端模式后，模型和账号数据经本项目后端，TokenONE Key 不得下发浏览器。
...
- 文档必须区分本地模式与可选云端模式：本地模式画布/素材在浏览器，云端模式按账号保存到独立 PostgreSQL 和文件卷；未通过真实联调的云端功能只记入待测清单。
- 本地模式 AI API Key 存在浏览器并直接请求接口；云端模式由服务端按用户身份和功能分组获取 TokenONE Key，请勿将两种安全边界混写。
