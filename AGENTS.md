...
- 前端服务请求统一放在 `web/src/services/api/`。仅支持登录后的云端模式，模型和账号数据经本项目后端，TokenONE Key 不得下发浏览器。
...
- 业务数据仅持久化到服务端，不使用浏览器数据库或本地草稿副本；浏览器只保留页面内存状态，`localStorage` 仅用于主题、面板布局等设备偏好和本地 Agent 连接配置。
...
- 在 CreativeOne 任务中不得直接修改 TokenONE 或其他关联项目的源码、构建或部署其修复版本；跨项目问题只提供诊断与修改建议，必须由用户在对应项目明确授权后处理。

- 新增或调整超时、重试次数、大小限制、并发上限等会改变实际行为的边界值前，必须先向用户说明适用环节、默认值和失败后的处理方式，并取得确认；不要把经验值当成纯内部实现静默加入。
- 不再支持本地模式；画布、素材、历史和业务配置按账号保存到 PostgreSQL 和文件卷，文档不得再宣称云端为可选模式；未通过真实联调的功能只记入待测清单。
- 模型调用由服务端按已验证用户身份获取 Enhance 应用默认分组的 TokenONE Key，浏览器不得配置或持久化上游 Key。
