...
+ [修复] 云端任务页的视频状态查询和下载适配 Ark 结果，媒体下载白名单支持包含根域名的泛域名规则。

+ [修复] 模型分组未启用生成权限时显示明确的配置指引，避免视频请求被拒绝后只显示泛化权限错误。

+ [优化] 模型列表手动刷新后显示候选数量与添加指引，明确刷新不会自动修改功能允许模型。

+ [调整] 视频创作统一图片工作室风格并恢复入口，视频调用切换为火山 Ark 任务协议，经服务端 TokenONE 鉴权并保存结果。

+ [调整] 移除本地模式和浏览器业务持久化，统一登录后读写云端；正常仅显示保存状态，失败可重试，冲突可导出未保存修改或加载云端。

+ [调整] 功能模型配置简化为图片、文本、视频、音频四项，每项独立维护可用模型、默认模型和启用状态。
+ [调整] 云端「功能渠道管理」更名为「功能模型配置」，统一导航与操作文案。
+ [调整] Enhance 应用接口切换到统一站点的 `/api/apps/*` 路径，移除对旧内部入口的依赖。
+ [新增] 云端渠道模型支持从 TokenONE 列表搜索多选，同时保留手动输入和批量粘贴。
+ [调整] 本地开发支持配置前端 API 代理地址，CreativeOne 后端可独立使用端口以避免与 Enhance 冲突。
+ [调整] 云端模型服务接入 TokenONE Enhance v2 应用凭证与默认分组，更新渠道管理和错误提示，并增加账户及本应用消耗查询接口。
+ [调整] TokenONE 账号改由 IDONE 注册同步，模型调用缺少账号时显示同步提示，应用侧不再提供自动建号开关。
...
+ [优化] 云端设置页采用紧凑分区与自适应明暗说明卡片，精简模型选择样式并单列管理员入口，隐藏关闭的视频及未配置音频选项。
+ [调整] 应用展示名称改为「画布ONE」，同步侧栏、登录页、页面标题及默认画布名称，保留存储标识和已有数据。
+ [调整] 暂时关闭创作助手、视频创作、无限画布和原版切换入口，精简生图操作，主题默认跟随系统并支持手动选择。
+ [优化] 模型失败显示余额、Key、分组及订阅等中文处理建议，生图卡片提供可展开的错误码和请求 ID，未知上游错误安全降级且不自动重试。
+ [调整] 本地 CreativeOne 默认生图渠道接入 TokenONE「生图测试」分组，完成真实用户 Key 创建与复用联调。
+ [新增] 提供独立本地 TokenONE 联调部署和 Enhancer 配置，CreativeOne 可连接本地服务，业务数据恢复及模型验收单独确认。
+ [调整] 支持显式启用同主机局域网 HTTP 开发联调，并将新增渠道 UUID 改由服务端生成，避免非安全浏览器上下文报错。
+ [修复] IDONE 支持应用专属 Discovery/JWKS 配置及受限 localhost HTTP 联调，继续严格校验身份与元数据 Issuer，并允许模型服务未配置时独立使用登录和存储。
+ [新增] 增加可选 IDONE 多用户后端、按功能分组的 TokenONE 用户 Key 路由、独立 PostgreSQL 与文件卷云同步，以及渠道管理和云端任务入口。
+ [新增] 新增可切换的 Studio 界面，提供侧栏导航、居中生图输入、模型设置浮层和灵感卡片，复用现有生成流程与本地记录。
