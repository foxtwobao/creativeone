import { CLOUD_ENABLED } from "@/constant/runtime-config";
...
const errors: Record<string, string> = {
    ...
    SYNC_CONFLICT: "其他设备已修改这份数据，本地草稿已保留，请导出草稿后重新加载云端版本",
...
export async function cloudApi<T>(path: string, init: RequestInit = {}): Promise<T> {
    ...
    if (!response.ok) {
        ...
        if (response.status === 401 || body.error === "CSRF_FAILED" || body.error === "ACCOUNT_CHANGED") useCloudStore.getState().setError("session", "登录状态已变化，请重新登录后恢复草稿");
        throw error;
    }
